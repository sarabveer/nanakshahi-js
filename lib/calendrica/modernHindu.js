...
const ayanamsha = tee => mod( solarLongitude( tee ) - siderealSolarLongitude( tee ), 360 )
...
const astroHinduFullmoonFromFixed = date => {
  ...
  let m
  if ( day >= 16 ) {
    m = astroHinduLunarFromFixed( date + 20 ).month
  } else {
    m = month
  }
  return { year, month: m, leapMonth, day, leapDay }
...
const astroHinduLunarStation = date => (
  Math.floor( lunarLongitude( universalFromStandard( altHinduSunrise( date ), HINDU_LOCATION ) )
    / angle( 0, 800, 0 ) ) - 1
)
