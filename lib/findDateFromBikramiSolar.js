 ...
 * `[astro=true]` Set to `false` to use Surya Sidhantta instead of Drik Gannit
 * @return {Object} Gregorian Date
 * @example findDateFromBikramiSolar( 1723, 9, 23 )
 */
function findDateFromBikramiSolar( year, month, date, { astro } = { astro: true } ) {
  ...
  const lunarDate = {
    ...
    punjabiDate: {
      month: toGurmukhiNum( lunarMonth ),
      monthName: lunarMonthName.pa,
      paksh: paksh.pa,
      tithi: toGurmukhiNum( tithi ),
      year: toGurmukhiNum( lunarYear ),
    },
  ...
  if ( fixedDay < 731288 ) {
    bikramiDate.nanakshahiYear = month === 12
      ? year - 1524
      : year - 1525
  }
