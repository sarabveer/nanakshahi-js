 ...
 * @param {Object} [gregorianDate] JavaScript Date() Object, defaults to current Date.
 * @return {Object} Tithi and Moon Info
 * @example getTithi( new Date() )
 */
function getTithi( gregorianDate = new Date() ) {
  // Get Moon Phase and Illumination
  const { phase, fraction: moonIllumination } = suncalc.getMoonIllumination( gregorianDate )

  // Get Tithi, Paksh, and Tithi Name
  const rawTithi = phase * 30
  let tithiDay = Math.floor( rawTithi ) + 1
  let paksh
  if ( tithiDay > 15 ) {
