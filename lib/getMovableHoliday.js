const getDateFromTithi = require( './getDateFromTithi' )
 ...
 */
module.exports = ( holiday, year = new Date().getFullYear() ) => {
  // Get data for event
  const holidayData = movableHolidays[ holiday ]
  const { month: lunarMonth, tithi, paksh: lunarPaksh } = holidayData.date
  ...
  const { gregorianDate, lunarDate } = getDateFromTithi( bikramiYear, lunarMonth, tithi, paksh ) // eslint-disable-line max-len
