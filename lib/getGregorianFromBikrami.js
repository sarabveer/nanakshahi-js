 ...
 * @example getGregorianFromBikrami( 1723, 10, 7 )
 */
function getGregorianFromBikrami( year, month, tithi, paksh = false ) {
  ...
  if ( finalDate === undefined ) {
    ...
    if ( finalDate === undefined ) {
      ...
      if ( finalDate === undefined ) {
        ...
        if ( finalDate === undefined ) {
          bikramiDate = getBikramiDate( new Date( gregorianYear, gregorianMonth, gregorianDay ) )
  ...
  // Lunar Date Obj
  const { nakshatra, tithiFraction } = bikramiDate.lunarDate
  const lunarDate = {
    ahargana,
    englishDate,
    punjabiDate,
  ...
  let gregorian = { // eslint-disable-line prefer-const
    ...
    julianDay,
    lunarDate,
    solarDate: bikramiDate.solarDate,
  }
