...
function findDateFromBikramiLunar(
  ...
  // Get name of Tithi
  const tithiName = ( date === 15 && paksh === true ) ? tithiNames[ 15 ] : tithiNames[ date - 1 ]
  ...
  const tithiStartFrac = astro
    ...
    : standardFromLocal( hinduLunarDayAtOrAfter( day, ( fixedDay - 2 ) ), HINDU_LOCATION )
  ...
  if ( julianDay < 2361221 ) {
    ...
    const sankrantiJulianDate = julianFromFixed( Math.floor( sankranti ) )
