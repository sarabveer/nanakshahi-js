 ...
 */
function getBikramiDate( gregorianDate, isJulian = false ) {
  let year
  let month
  let day
  let julianDay
  if ( isJulian === true ) {
    julianDay = fromJulianToGregorian( gregorianDate )
    year = julian.toDate( julianDay ).getFullYear()
  } else {
  ...
  let bikrami = { // eslint-disable-line prefer-const
    gregorianDate: julian.toDate( julianDay ),
    julianDay,
    lunarDate,
    solarDate,
    sunrise,
